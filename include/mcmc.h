...
// Run `num_sims` random rollouts from `board`, where the calling player's first
// move is forced to land on cell `player_move_pos` and every other move (all
// players, all subsequent steps) is sampled uniformly from free neighbours.
// Rollouts stop on death, win, or after `max_depth` steps. Results are returned
// raw, one entry per simulation; no aggregation/evaluation is done here.
